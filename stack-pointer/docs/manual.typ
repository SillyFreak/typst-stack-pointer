...
#let exec-grid-render(columns: none, steps) = {
  ...
  grid(
    columns: (1fr,) * columns,
    row-gutter: 1.4em,
    ..steps.enumerate(start: 1).map(((n, step)) => block(breakable: false, {
      [Step #n: ]
...
_Stack Pointer_ provides tools for visualizing program execution. Its main use case is for presentations using frameworks such as Polylux, but it tries to be as general as possible.
